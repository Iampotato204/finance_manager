...
int main(){
    try {
        ...
        printf("Content-Type: text/html\n\n"); // not the preferred solution, may cause difficulties with headers
...
//            cout<<elements.at(i).getName()<<endl;
            ...
            // memset and memcpy required, because data may vary in length, resulting in shorter arrays
            char tag[6];
            memset(tag, 0,6);
...
//            switch COULD be used, BUT this requires redefining equality operator for char array
            if (strcmp(tag,"valna")==0){// note name processed
                strcpy(new_note.text, elem.getValue().c_str());
            }else if (strcmp(tag,"valnu")==0){// note money processed
                new_val=atof(elem.getValue().c_str());
            }else if (strcmp(tag,"valtn")==0){
                strcpy(new_table.text, elem.getValue().c_str());
            }else if(strcmp(tag,"butnf")==0){// new row requested
            ...
            }else if (strcmp(tag,"butrf")==0){// delete row requested
                sprintf(tmpsql,"delete from transactions where id=%d;", atoi(fieldstr), new_val, new_note.text);
                rc = sqlite3_exec(db, tmpsql, 0, 0, &err_msg);
            }else if (strcmp(tag,"butnt")==0){// new table requested
    ...
    catch(exception& e) {printf("error in cgi: %s",e.what());}
